...
echo "[*] Installing Hyprland + Wayland environment..."
sudo pacman -S --noconfirm --needed \
    hyprland \
    waybar \
    dunst \
...
echo "[*] Linking Ax-Shell config..."
mkdir -p ~/.config/Ax-Shell/config
cp -r ~/Ax-Shell/config/* ~/.config/Ax-Shell/config/

echo "[*] Setting up Ax-Shell alias..."
if ! grep -q "ax-shell" ~/.bashrc; then
    echo "alias ax-shell='python ~/.config/Ax-Shell/config/config.py'" >> ~/.bashrc
fi
if ! grep -q "ax-shell" ~/.zshrc 2>/dev/null; then
    echo "alias ax-shell='python ~/.config/Ax-Shell/config/config.py'" >> ~/.zshrc
fi

echo "[*] Done! Reboot and login into Hyprland."
echo "   - Run 'ax-shell' to start Ax-Shell."
